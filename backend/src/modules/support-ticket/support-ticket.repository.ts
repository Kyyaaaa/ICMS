...
import { CreateSupportTicketDTO, CreateTicketMessageDTO, SupportTicket, TicketMessage } from './support-ticket.model';
...
export const SupportTicketRepository = {
    ...
    async getTicketsBySender(sender_id: string) {
        const query = `
            SELECT t.*, a.full_name as sender_name,
    ...
    async createMessage(data: CreateTicketMessageDTO) {
        ...
        try {
            ...
            const updateTicketQuery = `
                ...
                WHERE id = $1;
            `;
            await client.query(updateTicketQuery, [data.ticket_id]);
