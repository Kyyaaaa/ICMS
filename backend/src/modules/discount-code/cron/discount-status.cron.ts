  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      if (updateResult.rowCount && updateResult.rowCount > 0) {
        console.log(`[CronJob] Updated ${updateResult.rowCount} discount codes to Expired.`);
      }
      ...
      console.error('[CronJob Error] Failed to update discount code status:', error);
    }
  });
  
  console.log('[CronJob] Discount Status cron initialized.');
};
