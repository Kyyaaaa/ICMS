...
| UC-10 | Update Account Details          | Admin                               | Account Management       | Allows the Administrator to modify an existing user's information.                                                                                                                        |
...
| UC-13 | Create Learner/Tutor Account    | Staff                               | Account Management       | Allows Staff members to quickly register a new Learner or Tutor account directly from the management dashboard to support operational tasks.                                              |
...
| UC-16 | Update Classroom Status         | Admin                               | Facility Management      | Allows Admin to edit classroom details or change its operational status (e.g., Available, Under Maintenance).                                                                             |
...
| UC-24 | Approve Qualification           | Staff                               | Tutor Profile Management | Allows Staff members to verify the authenticity of a Tutor's uploaded certificates and officially approve or reject their qualification.                                                  |
| UC-25 | View Classes                    | Staff                               | Class Management         | Allows Staff to view the list of all currently active, upcoming, or completed offline classes.                                                                                            |
...
| Public Notifications        | X     | X       | X     | X     | X     |
| Register                    | X     |         |       |       |       |
| Login                       | X     | X       | X     | X     | X     |
| Forgot Password             | X     | X       | X     | X     | X     |
| Verify OTP                  | X     | X       | X     | X     | X     |
...
| Learner Attendance Progress |       | X       |       |       |       |
| Learner Schedules           |       | X       |       |       |       |
| Class Registration          |       | X       |       |       |       |
| Payment Checkout            |       | X       |       |       |       |
| Payment History             |       | X       |       |       |       |
| Refund Request              |       | X       |       |       |       |
| Learner Support Tickets     |       | X       |       |       |       |
| Tutor Dashboard             |       |         | X     |       |       |
| Tutor Profile               |       |         | X     |       |       |
| Tutor Qualifications        |       |         | X     |       |       |
| Availability Registration   |       |         | X     |       |       |
| Teaching Schedule           |       |         | X     |       |       |
| Class Attendance            |       |         | X     |       |       |
| Tutor Change Requests       |       |         | X     |       |       |
| Tutor Salary History        |       |         | X     |       |       |
| Tutor Support Tickets       |       |         | X     |       |       |
| Staff Dashboard             |       |         |       | X     |       |
| Consultation List           |       |         |       | X     |       |
| User Profile List           |       |         |       | X     |       |
| User Profile Detail         |       |         |       | X     |       |
| Manage Classes              |       |         |       | X     |       |
| Create Class                |       |         |       | X     |       |
| Staff Class Detail          |       |         |       | X     |       |
| Master Schedule             |       |         |       | X     |       |
| Tutor Availability          |       |         |       | X     |       |
| Staff Change Requests       |       |         |       | X     |       |
| Invoice List                |       |         |       | X     |       |
| Invoice Detail              |       |         |       | X     |       |
| Staff Manage Accounts       |       |         |       | X     |       |
| Staff Support Tickets       |       |         |       | X     |       |
| Staff Salary History        |       |         |       | X     |       |
| Staff Profile               |       |         |       | X     |       |
| Admin Dashboard             |       |         |       |       | X     |
| Admin Manage Courses        |       |         |       |       | X     |
| Admin Course Detail         |       |         |       |       | X     |
| Discount Codes              |       |         |       |       | X     |
| Manage Classrooms           |       |         |       |       | X     |
| Admin Manage Accounts       |       |         |       |       | X     |
| Admin Account Detail        |       |         |       |       | X     |
| Refund Management           |       |         |       |       | X     |
| Refund Detail               |       |         |       |       | X     |
...
| 2   | Class Management      | Auto Lock Past Attendance         | Automatically locks the attendance recording window for Tutors once the class session date has passed, preventing retrospective modification of attendance data without Staff approval.                         |
| 3   | Enrollment & Payment  | Auto Cancel Unpaid Registrations  | Automatically cancels class registrations and releases the reserved seats if the Learner fails to complete the payment transaction within 24 hours.                                                             |
| 4   | Enrollment & Payment  | Auto Reject Expired Refunds       | Automatically calculates the elapsed time from the payment timestamp and rejects any refund requests submitted beyond the allowed policy window (e.g., 24-hour window).                                         |
| 5   | Enrollment & Payment  | Auto Expire Discount Codes        | Automatically invalidates and disables discount codes when their predetermined expiration date passes or when the usage limits are exhausted.                                                                   |
| 6   | Financial & Payroll   | Auto Payroll Calculation          | A background cron job that runs automatically on the last day of the month to calculate salaries for Tutors based strictly on the number of sessions taught (not hourly rates) and substituted sessions.        |
| 7   | Dashboard & Analytics | Auto Sync System Analytics        | Automatically aggregates and updates the total revenue, total active learners, and payroll cost records in the analytics table to reflect real-time financial data for Admin and Staff Dashboards.              |
| 8   | Schedule Management   | Auto Resolve Schedule Conflicts   | An internal algorithm that automatically validates and prevents any scheduling actions that would result in a Tutor or a physical Classroom being assigned to overlapping time slots.                           |
| 9   | Security & Auth       | Auto Cleanup Expired Tokens       | Automatically scans and removes expired JWT refresh tokens, unused OTPs, and expired password reset links from the database to optimize storage and maintain system security.                                   |
| 10  | Notification Service  | Auto Trigger System Notifications | Automatically pushes in-app notifications upon specific system triggers (e.g., when Staff updates a schedule, when Admin creates an announcement, or when a refund is processed).                               |
| 11  | Authentication        | Auto-Redirect User Tracing        | Automatically saves the Guest's trace when they click "Register" on a specific Course Detail screen, and transparently redirects them to that exact Class Registration form immediately after successful login. |
| 12  | Support & Operations  | Auto Escalate Overdue Tickets     | Automatically escalates Learner/Tutor Support Tickets to Urgent status or flags them for Admin review if they remain unresolved by Staff for more than 48 hours.                                                |
