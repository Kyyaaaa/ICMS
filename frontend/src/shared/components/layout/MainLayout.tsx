...
export const MainLayout: React.FC<MainLayoutProps> = ({ role, title, basePath, navItems, renderSubTabs, titleIcon: TitleIcon = BookOpen }) => {
        ...
        const fetchNotifs = async () => {
            try {
                ...
                const mapped: NotificationItem[] = anns.map(ann => ({
                    ...
                    type: ann.audience.scope === 'System Wide' ? 'system' : role.toLowerCase()
                }));
        ...
        <div className="min-h-screen bg-[#f7fafc] flex font-sans text-[#181c1e]">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-65 bg-white border-r border-[#e0e3e5] transition-transform transform ${sidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'} md:sticky md:top-0 md:h-screen md:translate-x-0 flex flex-col`}>
                <div className="flex items-center justify-between h-18 px-6 border-b border-[#e0e3e5] shrink-0">
                    <Link to={basePath} className="text-2xl font-extrabold text-[#002045] flex items-center gap-2">
                        <TitleIcon className="w-7 h-7 text-[#0061a5]" />
                        ICMS <span className="text-[#0061a5] font-semibold text-lg">{title}</span>
                    </Link>
                ...
                <header className="sticky top-0 h-18 bg-white/80 backdrop-blur-md border-b border-[#e0e3e5] flex items-center justify-between px-6 shrink-0 z-30">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="hidden md:flex flex-col">
                            <span className="text-lg font-bold text-[#002045]">Welcome back, {fullName}!</span>
                            <span className="text-xs text-[#74777f]">System is running smoothly today.</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-5">
                        ...
                        <Link to={`${basePath.replace('/dashboard', '')}/profile`} className="flex items-center gap-3 pl-5 border-l border-[#e0e3e5] cursor-pointer group">
