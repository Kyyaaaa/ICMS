...
const CreateClass = () => {
  ...
  const occupiedGlobalSlots = useMemo(() => {
    ...
    allGlobalSessions.forEach((session) => {
      if (
        (tutor && session.tutor_id === tutor) ||
        (selectedRoom && session.classroom_id === selectedRoom.id)
      ) {
        if (session.date && session.slot) {
          ...
          if (
            !occupied.some(
              (o) => o.dayOfWeek === dayOfWeek && o.slot === session.slot,
            )
          ) {
  ...
  const filteredRooms = useMemo(() => {
    ...
    return rooms.filter((room) => {
      const conflicts = allGlobalSessions.some((session) => {
        ...
        return generatedSessions.some(
          (gs) => gs.date === session.date && gs.slot === session.slot,
        );
      });
      return !conflicts;
    });
    ...
    const loadData = async () => {
      try {
        ...
        if (isEdit && id) {
          ...
          setCapacity(classData.capacity || 30);
              ...
              if (session.date && session.slot) {
                ...
                if (
                  !existingSchedule.some(
                    (s) => s.dayOfWeek === dayOfWeek && s.slot === session.slot,
                  )
                ) {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      try {
        const targetCycle = await TutorAvailabilityService.getCycleByMonth(
          date.getMonth() + 1,
          date.getFullYear(),
        );
        if (targetCycle) {
          const availabilityData =
            await TutorAvailabilityService.getTutorProfiles(targetCycle.id);
  ...
  useEffect(() => {
    if (!startDate || !course || weeklySchedule.length === 0) {
      setGeneratedSessions((prev) => (prev.length > 0 ? [] : prev));
      return;
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in-up max-w-4xl pb-10">
      ...
      <div className="flex items-center gap-2 text-sm text-[#74777f]">
        <Link
          to="/staff/classes"
          className="hover:text-[#0061a5] transition-colors flex items-center gap-1"
        >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#e0e3e5] overflow-hidden">
        <div className="p-6 md:p-8 space-y-8">
          ...
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-[#002045] border-b pb-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <select
                  ...
                  className={`w-full px-4 py-3 border border-[#c4c6cf] rounded-xl focus:outline-none focus:border-[#0061a5] focus:ring-2 focus:ring-[#0061a5]/20 ${isEdit ? "bg-gray-100 cursor-not-allowed" : "bg-[#f8f9fa]"}`}
                >
                  ...
                  {allCourses.map((c) => {
                    ...
                    if (c.next_cohort) {
                      ...
                      if (
                        dateVal &&
                        dateVal < new Date().setHours(0, 0, 0, 0)
                      ) {
                        isStarted = true;
                      }
                    }
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-[#181c1e]">
                  Class Name (Code) <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={className}
                  onChange={(e) => setClassName(e.target.value)}
                  placeholder="e.g. IELTS-A03"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-[#181c1e] flex items-center gap-1">
                  <Users className="w-4 h-4 text-gray-500" /> Assign Tutor
                </label>
                <select
                  value={tutor}
                  onChange={(e) => {
              ...
              <div className="space-y-2 relative" ref={roomDropdownRef}>
                <label className="text-sm font-semibold text-[#181c1e] flex items-center gap-1">
                  <MapPin className="w-4 h-4 text-gray-500" /> Assign Room
                </label>
                <button
                  type="button"
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 shrink-0 text-gray-500 transition-transform ${isRoomDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
          ...
          <div className="border-t border-[#e0e3e5] pt-6 mt-6">
            <h2 className="text-xl font-bold text-[#002045] mb-4">
              3. Timeline
            </h2>
            ...
            {course && !startDate && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-amber-800">
                <p>
                  Please ensure the selected course has a valid Next Cohort
                  date, as the class timeline relies on it.
                </p>
              </div>
            )}
            ...
            <div>
              <label className="block text-sm font-semibold text-[#002045] mb-2">
                ...
                {isEdit && !hasLearners && (
                  <span className="ml-2 text-xs font-normal text-amber-600 italic">
                    Select slots to generate a new schedule and overwrite
                    existing sessions
                  </span>
                )}
              </label>
              ...
              {isEdit && hasLearners && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 text-sm text-blue-800 flex items-start gap-2 mt-2">
                  ...
                  <p>
                    This class already has enrolled learners. The weekly
                    schedule cannot be changed globally. To change a specific
                    session's schedule, please use the Class Details page.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-2 mt-2">
                {!course ? (
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center text-gray-500">
                    Please select a course program first to determine the
                    schedule timeline.
                  </div>
                ) : !tutor ? (
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-center text-gray-500">
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center text-amber-700">
                    This tutor has not registered any availability. Please
                    select another tutor.
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-500">
                        Select the weekly slots this class will occur on:
                      </span>
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        const daySlots = tutorSlots
                          .filter((s: string) => s.startsWith(`${dayName}-`))
                          .sort((a: string, b: string) => {
                            ...
                            const slotA =
                              shiftToSlotMap[a.split("-")[1]] ||
                              a.split("-")[1];
                            const slotB =
                              shiftToSlotMap[b.split("-")[1]] ||
                              b.split("-")[1];
                        ...
                        return (
                          <div
                            key={dayName}
                            className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-start sm:items-center border-b border-gray-100 pb-3 last:border-0 last:pb-0"
                          >
                            <span className="text-sm font-bold text-[#002045] w-24 shrink-0">
                              {dayName}
                            </span>
                            <div className="flex flex-wrap gap-2">
                              {daySlots.map((slotStr: string) => {
                                ...
                                const isSelected = weeklySchedule.some(
                                  (s) =>
                                    s.dayOfWeek === dayIndex &&
                                    s.slot === slotCode,
                                );
                                const isOccupied = occupiedGlobalSlots.some(
                                  (s) =>
                                    s.dayOfWeek === dayIndex &&
                                    s.slot === slotCode,
                                );
                                ...
                                return (
                                  <button
                                    ...
                                    title={
                                      isOccupied
                                        ? "Tutor is already busy at this time for another class"
                                        : ""
                                    }
                                  >
                                    {label}
                                    {isOccupied && (
                                      <span className="absolute -top-1 -right-1 flex w-3 h-3 bg-red-500 border-2 border-white rounded-full"></span>
                                    )}
                                  </button>
                                );
                              })}
        ...
        <div className="p-6 bg-[#f8f9fa] border-t border-[#e0e3e5] flex justify-end gap-3">
          <Link
            to="/staff/classes"
            className="px-6 py-3 font-semibold text-[#43474e] border border-[#c4c6cf] rounded-xl hover:bg-white transition-colors"
          >
            Cancel
          </Link>
          <button
            ...
            onClick={async () => {
              ...
              if (selectedRoom && capacity > selectedRoom.capacity) {
                showAlertModal(
                  "Error",
                  `Class capacity (${capacity}) cannot exceed the selected room's capacity (${selectedRoom.capacity}).`,
                  "error",
                );
                return;
              }
              ...
              try {
                ...
                showAlertModal(
                  "Error",
                  (err as Error).message || "Error saving class",
                  "error",
                );
