...
import { AlertCircle, ChevronRight, FileText, MessageSquare, DollarSign, CheckCircle } from 'lucide-react';
import type { StaffPendingTask } from '../types/dashboard';

interface DashboardPendingTasksProps {
    tasks: StaffPendingTask[];
}
            ...
            <div className="space-y-4">
                {tasks.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-8 text-center text-[#74777f]">
                        ...
                        <p className="font-bold text-sm text-[#181c1e]">Inbox zero!</p>
                        <p className="text-xs mt-1">All tickets and invoices have been resolved.</p>
                    </div>
                )}
                {tasks.map((task, i) => {
