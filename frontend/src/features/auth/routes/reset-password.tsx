    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 md:p-8 font-sans antialiased bg-[#002045] relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-120">
                ...
                {/* Main Card */}
                <main className="bg-white w-full rounded-3xl shadow-[0_24px_64px_rgba(0,0,0,0.4)] p-8 md:p-12 border border-white/20 overflow-hidden min-h-125 animate-fade-in-up">
                    
                    {/* View 1: Reset Password Form */}
                    <div className={`transition-opacity duration-300 flex flex-col h-full ${isSuccess ? 'opacity-0 pointer-events-none absolute inset-0 p-10' : 'opacity-100 relative'}`}>
                        <div className="text-center mb-6">
                            <h2 className="text-2xl leading-8 font-bold text-[#181c1e] mb-2 tracking-tight">Set new password</h2>
                            <p className="text-sm leading-5.5 text-[#43474e] max-w-80 mx-auto">
                                Must be different from previously used passwords.
                            </p>
                        </div>
                        
                        <form className="flex flex-col gap-4 grow" onSubmit={handleSubmit}>
                            {/* New Password Input */}
                            <div className="flex flex-col gap-1.5 relative">
                                <label className="text-xs font-semibold text-[#181c1e]" htmlFor="new-password">New Password</label>
                                <div className="relative w-full">
                                    <input 
                                        className="w-full h-12 px-4 bg-[#f7fafc] border border-[#c4c6cf] rounded-xl text-sm text-[#181c1e] placeholder-[#74777f] focus:outline-none focus:border-[#0061a5] focus:ring-4 focus:ring-[#0061a5]/10 transition-all shadow-sm hover:border-[#74777f]" 
                                        id="new-password" 
                                    ...
                                    />
                                    <button 
                                        className="absolute right-4 top-1/2 -translate-y-1/2 text-[#43474e] hover:text-[#002045] transition-colors focus:outline-none" 
                                        type="button"
                                ...
                                <ul className="mt-2 space-y-1">
                                    <li className={`flex items-center gap-2 text-sm leading-5 transition-colors ${isLength ? 'text-[#181c1e]' : 'text-[#43474e]'}`}>
                                    ...
                                    <li className={`flex items-center gap-2 text-sm leading-5 transition-colors ${isCases ? 'text-[#181c1e]' : 'text-[#43474e]'}`}>
                                        {isCases ? <CheckCircle2 className="w-4 h-4 text-[#0061a5] fill-[#0061a5]/20" /> : <Circle className="w-4 h-4 text-[#74777f]" />}
                                        Contains uppercase and lowercase letters
                                    </li>
                                    <li className={`flex items-center gap-2 text-sm leading-5 transition-colors ${isNumber ? 'text-[#181c1e]' : 'text-[#43474e]'}`}>
                                        {isNumber ? <CheckCircle2 className="w-4 h-4 text-[#0061a5] fill-[#0061a5]/20" /> : <Circle className="w-4 h-4 text-[#74777f]" />}
                                        Contains a number
                                    </li>
                                    <li className={`flex items-center gap-2 text-sm leading-5 transition-colors ${isSpecial ? 'text-[#181c1e]' : 'text-[#43474e]'}`}>
                            ...
                            <div className="flex flex-col gap-1.5 relative">
                                <label className="text-xs font-semibold text-[#181c1e]" htmlFor="confirm-password">Confirm New Password</label>
                                <div className="relative w-full">
                                    <input 
                                        className={`w-full h-12 px-4 bg-[#f7fafc] border rounded-xl text-sm text-[#181c1e] placeholder-[#74777f] focus:outline-none focus:border-[#0061a5] focus:ring-4 focus:ring-[#0061a5]/10 transition-all shadow-sm hover:border-[#74777f] ${showMatchError ? 'border-[#ba1a1a]' : 'border-[#c4c6cf]'}`}
                                        id="confirm-password" 
                                ...
                                {showMatchError && (
                                    <p className="text-sm leading-5 text-[#ba1a1a] mt-1 flex items-center gap-1 animate-fade-in">
                            ...
                            {errorMsg && (
                                <div className="text-sm leading-5 text-[#ba1a1a] flex items-center gap-1.5 animate-fade-in font-medium">
                    ...
                    <div className={`absolute inset-0 bg-white flex flex-col items-center justify-center p-10 text-center transition-opacity duration-500 ${isSuccess ? 'opacity-100 pointer-events-auto z-10' : 'opacity-0 pointer-events-none'}`}>
                        <div className={`w-20 h-20 rounded-2xl bg-[#f7fafc] border border-[#e0e3e5] shadow-sm flex items-center justify-center mb-6 transition-transform duration-500 delay-100 ${isSuccess ? 'scale-100' : 'scale-0'}`}>
                            <CheckCircle2 className="text-[#0061a5] w-10 h-10" />
                        </div>
                        <h2 className={`text-2xl leading-8 font-bold text-[#181c1e] mb-3 tracking-tight transform transition-all duration-500 delay-200 ${isSuccess ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                            All done!
                        </h2>
                        <p className={`text-sm leading-6 text-[#43474e] max-w-80 mx-auto mb-10 transform transition-all duration-500 delay-300 ${isSuccess ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                            Your password has been securely updated. You can now use your new credentials to log into the management system.
                        </p>
                        <Link 
                            className={`w-full h-13 bg-linear-to-r from-[#003c71] to-[#0061a5] text-white text-sm font-semibold rounded-xl flex items-center justify-center gap-2 transition-all duration-500 delay-400 hover:shadow-[0_12px_24px_rgba(0,97,165,0.3)] hover:-translate-y-0.5 active:scale-[0.98] transform ${isSuccess ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'} group/btn`} 
                            to="/login"
                        >
                            Continue to log in
