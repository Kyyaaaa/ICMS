    ...
    return (
        <div className="space-y-6 animate-fade-in-up pb-8">
            ...
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl md:text-3xl font-bold text-[#002045]">Course Details</h1>
            ...
            {isEditing ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white rounded-xl shadow-sm border border-[#e0e3e5] overflow-hidden">
                            <div className="p-6 border-b border-[#e0e3e5] bg-[#f8f9fa]">
                                <label className="block text-xs font-bold text-[#43474e] mb-2">Cover Image</label>
                                <div className="flex gap-4 items-start">
                                    ...
                                    <div className="flex-1">
                                        <p className="text-xs text-[#74777f] mb-2">Upload a high-quality image to represent this course. Recommended size: 1200x800px (16:9 ratio). Max file size: 5MB.</p>
                                    </div>
                                </div>
                            </div>
                            <div className="p-6">
                                <div className="flex flex-col mb-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <select name="status" value={courseData.status} onChange={handleChange} className="w-full px-3 py-2 text-sm border border-[#c4c6cf] rounded-lg focus:outline-none focus:border-[#0061a5] bg-white">
                                ...
                                <div className="mb-6">
                                    <textarea name="description" value={courseData.description} onChange={handleChange} className="w-full text-[#43474e] leading-relaxed p-3 border border-[#c4c6cf] rounded-lg focus:outline-none focus:border-[#0061a5] min-h-25" placeholder="Course Description"></textarea>
                                </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-white rounded-xl shadow-sm border border-[#e0e3e5] p-6">
                            <h3 className="text-lg font-bold text-[#181c1e] mb-4">Pricing & Cohort</h3>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-[#e0e3e5] p-6">
                            <h3 className="text-lg font-bold text-[#181c1e] mb-4">Course Settings</h3>
                            <div>
                                <label className="block text-xs font-bold text-[#43474e] mb-1">Max Class Size</label>
                                <div className="flex items-center gap-1">
                                    <input type="number" name="maxSize" value={courseData.maxSize} onChange={handleChange} className="w-full text-sm font-bold px-2 py-1.5 border border-[#c4c6cf] rounded-lg focus:outline-none focus:border-[#0061a5]" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#002045] rounded-3xl p-6 md:p-10 shadow-lg mb-10 relative overflow-hidden flex flex-col md:flex-row gap-10 items-center">
                        ...
                        <div className="flex-1 z-10 w-full">
                            <div className="flex flex-wrap gap-2 mb-4">
                                <span className="bg-[#0061a5] text-white text-xs font-bold px-3 py-1 rounded-full">{courseData.category || 'Category'}</span>
                                <span className={`text-xs font-bold px-3 py-1 rounded-full ${courseData.status === 'Active' ? 'bg-[#e6f4ea] text-[#137333]' : courseData.status === 'Hidden' ? 'bg-[#ffebed] text-[#ba1a1a]' : 'bg-[#f1f4f6] text-[#74777f]'}`}>
                                    {courseData.status}
                                </span>
                            ...
                            <h1 className="text-3xl md:text-5xl font-extrabold text-white mb-4 leading-tight tracking-tight">{courseData.title}</h1>
                            <p className="text-lg text-[#adc7f7] max-w-2xl mb-8 leading-relaxed">{courseData.description}</p>
                        ...
                        <div className="bg-white rounded-2xl p-8 shadow-xl w-full md:w-85 z-10 flex flex-col border border-[#e0e3e5]">
                            <div className="flex justify-between items-end mb-4">
                                <span className="text-4xl font-extrabold text-[#002045] leading-none">{courseData.price} đ</span>
                                {courseData.originalPrice && (
                                    <span className="text-lg text-[#74777f] line-through font-medium mb-1">{courseData.originalPrice} đ</span>
                                )}
                            </div>
                                    ...
                                    {courseData.modules && courseData.modules.map((module, index) => (
                                        <div key={index} className={`bg-white border border-[#c4c6cf] rounded-2xl overflow-hidden shadow-sm ${index > 0 ? 'opacity-70' : ''}`}>
                                            <div className="bg-[#f7fafc] px-6 py-4 flex justify-between items-center border-b border-[#e0e3e5]">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${index === 0 ? 'bg-[#0061a5] text-white' : 'bg-[#e0e3e5] text-[#43474e]'}`}>
                                                        {index + 1}
                                                    </div>
                                                    <h3 className={`text-lg font-bold ${index === 0 ? 'text-[#002045]' : 'text-[#43474e]'}`}>{module.title}</h3>
                                                </div>
                                                <div className="flex items-center gap-4">
