                        ...
                        {logs.map(log => (
                            <tr key={log.id} className="border-b border-[#e0e3e5] last:border-0 hover:bg-[#f7fafc]">
                                <td className="py-4 px-6">
                                    <div className="text-[14px] font-bold text-[#181c1e]">{log.date}</div>
                                </td>
                                ...
                                <td className="py-4 px-6">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-full text-white flex items-center justify-center font-bold text-[12px] ${log.type === 'system' ? 'bg-[#43474e]' : log.adminRole === 'Super Admin' ? 'bg-[#0061a5]' : 'bg-[#fceeee] text-[#ba1a1a]'}`}>
                                            {log.adminInitials}
                                        </div>
                                        <div>
                                            <div className="text-[14px] font-bold text-[#002045]">{log.adminName}</div>
                                            <div className="text-[12px] text-[#74777f]">{log.adminRole}</div>
                                        </div>
                                    </div>
                                </td>
                                <td className="py-4 px-6">
                                    <span className="text-[14px] text-[#43474e]">{log.actionDetails}</span>
                                </td>
                            </tr>
                        ))}
                    </tbody>
